    ...
    const carregarDados = async () => {
        try {
            ...
            const [preRes, contraRes, agendRes, salasRes] = await Promise.all([
                ...
                fetch(`${Config.api_url}/api/salas/list`)
            ]);
            ...
            // Primeiro tentar buscar salas pela rota admin
            ...
            if (salasResponse.ok) {
                ...
                if (salasData.sucesso) {
                    setSalas(salasData.data);
                } else if (salasData.produtos && salasData.produtos[0]?.variacoes) {
                    const todasSalas = [];
                    salasData.produtos[0].variacoes.forEach(andar => {
                        andar.variacoes.forEach((sala, index) => {
                            todasSalas.push({
                                id: sala.id,
                                andar: parseInt(andar.atributos.andar[0].valor),
                                numero: (index + 1).toString(),
                                nome: sala.atributos.nome[0].valor,
                                area: parseFloat(sala.atributos.area[0].valor),
                                posicao: sala.atributos.posicao[0].valor,
                                preco: parseFloat(sala.precos.de[0].valor),
                                disponivel: sala.atributos.disponibilidade[0].valor,
                                imagem: sala.arquivos.imagens[0]?.baixar?.replace('/uploads/seedImg/', ''),
                                planta: sala.arquivos.plantas[0]?.baixar?.replace('/uploads/seedPlanta/', '')
                            });
                        });
                    });
                    setSalas(todasSalas);
